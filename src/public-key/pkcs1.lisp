...
(defvar *hash-algorithm-digest-info* '(:md2    #(48 32 48 12 6 8 42 134 247 13 2 2 5 0 4 16)
                                       :md5    #(48 32 48 12 6 8 42 134 247 13 2 5 5 0 4 16)
                                       :sha1   #(48 33 48 9 6 5 43 14 3 2 26 5 0 4 20)
                                       :sha256 #(48 49 48 13 6 9 96 134 72 1 101 3 4 2 1 5 0 4 32)
                                       :sha384 #(48 65 48 13 6 9 96 134 72 1 101 3 4 2 2 5 0 4 48)
                                       :sha512 #(48 81 48 13 6 9 96 134 72 1 101 3 4 2 3 5 0 4 64)))

(defun pkcs1-encode (digest-name message num-bytes)
  (let* ((digest-name (if (eq digest-name t) :sha1 digest-name))
         (h (digest-sequence digest-name message))
         (digest-info (getf *hash-algorithm-digest-info* digest-name))
         (der-length (+ (length h) (length digest-info)))
         (ps-length (- num-bytes der-length 3)))
    (assert (>= num-bytes (+ der-length 11)))
    (concatenate '(vector (unsigned-byte 8))
                 #(1)
                 (make-array ps-length :initial-element 255)
                 #(0)
                 digest-info
                 h)))
